...
multiomics <- rbind(
  fread("../../Dataset/Scaled/Metabolomics.txt") %>%
    pivot_longer(2:ncol(.)) %>%
    mutate(view = "metabolomics"),
  fread("../../Dataset/Scaled/Lipidomics_Positive.txt") %>%
    pivot_longer(2:ncol(.)) %>%
    mutate(view = "lipidomics positive"),
  fread("../../Dataset/Scaled/Lipidomics_Negative.txt") %>%
    pivot_longer(2:ncol(.)) %>%
    mutate(view = "lipidomics negative")
) %>%
...
#
#fdata = fread("../../Dataset/Scaled/FData.csv")
#
##splits = fdata %>%
...
#sample_order <- fread("../../Dataset/Scaled/Metabolomics.txt") %>%
#  colnames() %>%
#  .[2:35]
#
...
#  mofapath <- paste0("/Users/degn400/Downloads/MOFA_Mods/", mofaname, ".hdf5")
#  
#  # Train MOFA 
...
u_mat = get_factors(MOFAobject.trained, as.data.frame = T) %>%
  ...
  mutate(
    group = ifelse(grepl("N", sample), "None", "Surrounding")
  ) %>%
  select(-sample)
...
predict_prob <- function(model, newdata){
  stats::predict(model, newdata, type = "prob")$`.pred_None`
}
...
v_mat <- do.call(rbind, lapply(1:3, function(num) {
...
## Explore the UMat Separation

```{r}
u_mat %>%
  ggplot(aes(x = Factor6, y = Factor5, color = group)) +
    geom_point(size = 2)
```
