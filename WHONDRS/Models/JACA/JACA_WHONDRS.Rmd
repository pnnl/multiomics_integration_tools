...
responseY = model.matrix(~matrix(factor(colnames(metab)[-1] %>% substr(1,1), levels = c("N", "S")), ncol = 1)-1)
colnames(responseY) = c("N", "S")
```
...
#                        lambda = rep(cv_jacaparams$lambda, times = 3),
...
#                           mutate(group = factor(group, levels = c("N", "S"))) %>% 
#                           dplyr::select(group)) %>%
#        test_metrics(truth = group, .pred_N, estimate = .pred_class) %>%
...
# pr_auc  all_nn_19        0.7           0.1     0.01 all_nn_1_7          1            1 0.0000000001     10
#final_jacamod <- jacaTrain(Z = responseY, 
#                            X_list = omicsX, 
#                            lambda = rep(0.1, times = 3),
#                            rho = 0.01,
#                            missing = FALSE,
#                            alpha = 0.7,
#                            kmax = 100000,
...
v_mat <- do.call(rbind, final_jacamod) %>%
  ...
  mutate(
    Factor = "Factor1",
    Feature = c(metab$Feature, lip_pos$Feature, lip_neg$Feature),
